...
- `skoda/enyaq/battery/soc` - Battery state of charge (%)
- `skoda/enyaq/battery/range` - Remaining range (km)
- `skoda/enyaq/battery/charging` - Charging status (ON/OFF)
- `skoda/enyaq/battery/plugged_in` - Plugged in status (ON/OFF)
- `skoda/enyaq/doors/locked` - Door lock status (LOCKED/UNLOCKED)
- `skoda/enyaq/availability` - Bridge availability (online/offline)
- `skoda/enyaq/last_updated` - Last update timestamp

### Command Topics (Subscribed by bridge)

- `skoda/enyaq/command/start_charging` - Start charging
- `skoda/enyaq/command/stop_charging` - Stop charging
- `skoda/enyaq/command/lock` - Lock/unlock vehicle (payload: LOCK/UNLOCK)
